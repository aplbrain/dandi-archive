...
def test_garbage_collect_uploads(models_and_factories):
    ...
    expired_upload: Upload = models_and_factories.upload_factory()
    expired_upload.created = timezone.now() - garbage_collection.UPLOAD_EXPIRATION_TIME
    expired_upload.save()

    non_expired_upload: Upload = models_and_factories.upload_factory()

    assert garbage_collection.upload.garbage_collect() == 1

    assert models_and_factories.UploadModel.objects.filter(id=non_expired_upload.id).exists()
    assert not models_and_factories.UploadModel.objects.filter(id=expired_upload.id).exists()


@pytest.mark.django_db
def test_garbage_collect_asset_blobs(asset_factory, models_and_factories):
    # Case 1: AssetBlob is orphaned, and older than the expiration time
    orphaned_expired_asset_blob: AssetBlob = models_and_factories.blob_factory()
    ...
    orphaned_non_expired_asset_blob: AssetBlob = models_and_factories.blob_factory()

    # Case 3: AssetBlob is not orphaned, but is older than the expiration time
    non_orphaned_expired_asset_blob: AssetBlob = models_and_factories.blob_factory()
    ...
    non_orphaned_non_expired_asset_blob: AssetBlob = models_and_factories.blob_factory()
    ...
    assert not models_and_factories.BlobModel.objects.filter(
        id=orphaned_expired_asset_blob.id
    ).exists()
    assert models_and_factories.BlobModel.objects.filter(
        id=orphaned_non_expired_asset_blob.id
    ).exists()
    assert models_and_factories.BlobModel.objects.filter(
        id=non_orphaned_expired_asset_blob.id
    ).exists()
    assert models_and_factories.BlobModel.objects.filter(
...
@pytest.mark.django_db
def test_garbage_collection_event_records(models_and_factories, mocker):
    ...
    for _ in range(asset_blob_count):
        asset_blob: AssetBlob = models_and_factories.blob_factory()
    ...
    for _ in range(upload_count):
        upload: Upload = models_and_factories.upload_factory()
    ...
    assert all(
        not models_and_factories.BlobModel.objects.filter(id=asset_blob.id).exists()
        for asset_blob in garbage_collected_asset_blobs
    )
    assert all(
        not models_and_factories.UploadModel.objects.filter(id=upload.id).exists()
    ...
    assert (
        GarbageCollectionEvent.objects.filter(type=models_and_factories.BlobModel.__name__).count()
        == 1
    )
    assert (
        GarbageCollectionEvent.objects.filter(
            type=models_and_factories.UploadModel.__name__
        ).count()
        == 1
    )
    ...
    assert (
        GarbageCollectionEventRecord.objects.filter(
            event__type=models_and_factories.BlobModel.__name__
        ).count()
        == asset_blob_count
    )
    assert (
        GarbageCollectionEventRecord.objects.filter(
            event__type=models_and_factories.UploadModel.__name__
        ).count()
        == upload_count
    )

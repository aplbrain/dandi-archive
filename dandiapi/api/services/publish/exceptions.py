...
class DandisetAlreadyPublishingError(DandiError):
    ...
    message = 'EMBERset is currently being published'


class DandisetBeingValidatedError(DandiError):
    http_status_code = status.HTTP_409_CONFLICT
    message = 'EMBERset is currently being validated'


class DandisetInvalidMetadataError(DandiError):
    http_status_code = status.HTTP_400_BAD_REQUEST
    message = 'EMBERset metadata or asset metadata is not valid'


class DandisetValidationPendingError(DandiError):
    http_status_code = status.HTTP_409_CONFLICT
    message = 'Metadata validation is pending for this EMBERset, please try again later.'
